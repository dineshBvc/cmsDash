...
export class ProductsPage {
  ...
  loadCategories() {
    this.apiService.GetCategories(this.appType).subscribe(
      (categories: any) => {
  ...
  GetProducts() {
    this.apiService.GetProducts(this.appType).subscribe((data: Product[]) => {
  ...
  AddProduct() {
    ...
    if (this.newProductForm.valid) {
      ...
      if (this.productId.length === 0) {
        this.apiService.AddProduct(formData, this.appType).subscribe(
          (response: any) => {
        ...
        this.apiService.EditProduct(this.productId, formData, this.appType).subscribe(
          (response: any) => {
            this.alertService.successToast(response.message);
            this.Reset();
          });
