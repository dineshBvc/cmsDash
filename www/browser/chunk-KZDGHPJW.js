...
// src/app/services/signlar.service.ts
var _SignlarService = class _SignlarService {
  constructor(alertService) {
    this.alertService = alertService;
    this.machineStatus = new BehaviorSubject({ id: "", connected: false, status: "", updatedon: /* @__PURE__ */ new Date() });
    this.liveReportReceivedStatus = new BehaviorSubject({ requestId: "", message: "" });
    this.logReceivedStatus = new BehaviorSubject({ requestId: "", message: "" });
  }
  Connect() {
    this.hubConnection = new HubConnectionBuilder().withUrl(`${environment.apiURL}/hubs/notification`, {
    ...
    this.hubConnection.on("VendingMachineStatus", (status) => {
      this.machineStatus.next(status);
    });
    this.hubConnection.on("ReportResponse", (status) => {
      this.alertService.successToast(status.message);
      this.liveReportReceivedStatus.next(status);
    });
    this.hubConnection.on("LogResponse", (status) => {
      this.alertService.successToast(status.message);
...
_SignlarService.\u0275fac = function SignlarService_Factory(t) {
  return new (t || _SignlarService)(\u0275\u0275inject(AlertService));
};
_SignlarService.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _SignlarService, factory: _SignlarService.\u0275fac, providedIn: "root" });
var SignlarService = _SignlarService;

// src/app/services/machine.service.ts
var _MachineService = class _MachineService {
  constructor() {
    this.api = inject(ApiService);
    this.signalrService = inject(SignlarService);
    this.machines = [];
    this.loaded = false;
    this.machineStatus = new BehaviorSubject([]);
  ...
  LoadAllMachines() {
    this.loaded = false;
    this.api.GetMachinesStatus().subscribe((result) => {
  ...
  ngOnDestroy() {
    if (this.machineStatusSub)
      this.machineStatusSub.unsubscribe();
  }
  delay(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
  }
};
_MachineService.\u0275fac = function MachineService_Factory(t) {
  return new (t || _MachineService)();
};
_MachineService.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _MachineService, factory: _MachineService.\u0275fac, providedIn: "root" });
var MachineService = _MachineService;
